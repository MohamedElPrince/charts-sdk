...
var formatDate = utcFormat('%Y');
var n = 12;
var k = 10;
var barSize = 48;
function bars(svg, x, y, data) {
  var bar = svg.append('g').attr('fill-opacity', 0.6).selectAll('rect');
  var color = getColor(data);
  return function (_ref, transition, prev, next) {
    var data = _ref[1];
    return bar = bar.data(data.slice(0, n), function (d) {
      return d.name;
    }).join(function (enter) {
      return enter.append('rect').attr('fill', color).attr('height', y.bandwidth()).attr('x', x(0)).attr('y', function (d) {
...
function labels(svg, x, y) {
  ...
  return function (_ref2, transition, prev, next) {
    var data = _ref2[1];
...
function axis(svg, x, y, width) {
  var g = svg.append('g').attr('transform', "translate(0," + margin.top + ")");
  var axis = axisTop(x).ticks(width / 160).tickSizeOuter(0).tickSizeInner(-barSize * (n + y.padding()));
...
function ticker(svg, width, keyframes) {
  var now = svg.append('text').style('font', "bold " + barSize + "px var(--sans-serif)").style('font-variant-numeric', 'tabular-nums').attr('text-anchor', 'end').attr('x', width - 6).attr('y', margin.top + barSize * (n - 0.45) - 70).attr('dy', '0.32em').text(formatDate(keyframes[0][0]));
  return function (_ref3, transition) {
    var date = _ref3[0];
...
var getKeyframes = function getKeyframes(data) {
  var datevalues = Array.from(rollup(data, function (_ref4) {
    var d = _ref4[0];
    return d.value;
  }, function (d) {
    return d.date;
  ...
  for (var _iterator = _createForOfIteratorHelperLoose(pairs(datevalues)), _step; !(_step = _iterator()).done;) {
    var _step$value = _step.value;
    var _step$value$ = _step$value[0];
    ka = _step$value$[0];
    a = _step$value$[1];
    var _step$value$2 = _step$value[1];
    kb = _step$value$2[0];
    b = _step$value$2[1];
...
function BarChart(_ref) {
  var _ref$dimension = _ref.dimension,
      width = _ref$dimension.width,
      height = _ref$dimension.height,
      _ref$insight = _ref.insight,
      data = _ref$insight.data,
      context = _ref$insight.context;
  var duration = context.insight.settings.duration;
  ...
  var ref = useD3(function (svg) {
    try {
      ...
      var x = linear([0, 1], [margin.left, width - margin.right]);
      var y = band().domain(range(n + 1)).rangeRound([margin.top, margin.top + barSize * (n + 1 + 0.1)]).padding(0.1);
      svg.attr('viewBox', [0, 0, width, height]);
      var keyframes = getKeyframes(data);
      var updateBars = bars(svg, x, y, data);
      var updateAxis = axis(svg, x, y, width);
      var updateLabels = labels(svg, x, y);
      var updateTicker = ticker(svg, width, keyframes);

      var _temp2 = _forOf(keyframes, function (keyframe) {
        var nameframes = groups(keyframes.flatMap(function (_ref2) {
          var data = _ref2[1];
          return data;
        }), function (d) {
          return d.name;
        });
        var prev = new Map(nameframes.flatMap(function (_ref3) {
          var data = _ref3[1];
          return pairs(data, function (a, b) {
            return [b, a];
          });
        }));
        var next = new Map(nameframes.flatMap(function (_ref4) {
          var data = _ref4[1];
          return pairs(data);
        }));
        var transition = svg.transition().duration(duration).ease(linear$1);
        x.domain([0, keyframe[1][0].value]);
        updateAxis(keyframe, transition);
        updateBars(keyframe, transition, prev, next);
        updateLabels(keyframe, transition, prev, next);
        updateTicker(keyframe, transition);
        return Promise.resolve(transition.end()).then(function () {});
      });
